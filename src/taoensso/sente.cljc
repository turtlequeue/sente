...
(defn make-channel-socket-server!
  ...
  (have? #(satisfies? interfaces/IPacker %) packer)
  ...
  (let [allowed-origins (have [:or set? #{:all}] allowed-origins)
        ch-recv (chan recv-buf-or-n) ;; buf or n ??? eww
        user-id-fn
        (fn final-user-id-fn [ring-req client-id]
          ;; Allow uid to depend (in part or whole) on client-id. Be cautious
          ;; of security implications.
          (or (user-id-fn (assoc ring-req :client-id client-id)) :turtlequeue/nil-uid))
        ...
        (fn [user-id ev & [{:as opts :keys [flush?]}]]
          (let [uid (if (= user-id :turtlequeue/all-users-without-uid) :turtlequeue/nil-uid user-id)
                _   (tracef "Chsk send: (->uid %s) %s" uid ev)
                _   (assert uid
                            (str "Support for sending to `nil` user-ids has been REMOVED. "
                                 "Please send to `:turtlequeue/all-users-without-uid` instead."))
                _   (assert-event ev)
    ...
    {:ch-recv        ch-recv
     ...
     ;; TODO make a version that takes a yada `ctx`
     ;; for this:
     ;; - make this a parameter
     ;; - override it with a similar impl

     :ajax-get-or-ws-handshake-fn
     (fn ajax-get-or-ws-handshake-fn [{:keys [ring-req] :as ctx}]
       (let [sch-uuid   (enc/uuid-str 6)
             params     (get ring-req :params)
             client-id  (get params   :client-id)
             uid        (user-id-fn   ctx client-id)
             ...
             (fn [server-ch websocket?]
               ...
               (let [?handshake-data (handshake-data-fn ctx)
                     handshake-ev
                     (if (nil? ?handshake-data) ; Micro optimization
                       ;; second nil was csrf-token
         ...
         (cond
           (str/blank? client-id)
           (let [err-msg "Missing clientId"
                 ;;"Client's Ring request doesn't have a client id."
                 ;;" Does your server have the necessary keyword Ring middleware (`wrap-params` & `wrap-keyword-params`)?"
                 ]
                                        ...
                                        ; Client will immediately repoll
                                                 ...
                                                 (fn [server-ch websocket? error]
                                                   (errorf "ring-req->server-ch-resp error: %s (%s)"
                                                           error uid sch-uuid))}))))
     #_(fn ajax-get-or-ws-handshake-fn [ring-req]
       (let [sch-uuid   (enc/uuid-str 6)
             params     (get ring-req :params)
             client-id  (get params   :client-id)
             uid        (user-id-fn    ring-req client-id)
...
;; default JS client Chan
#?(:cljs
   (defrecord JSDefaultWebSocket [socket]
     interfaces/IClientWebSocket
     (cws-close! [this]
       (when socket
         (.close socket 1000 "CLOSE_NORMAL")))
     (cws-send! [this msg]
       (when socket
         (.send socket msg)))))

#?(:cljs (defn js-websocket-creator [url {:keys [on-msg on-error on-close]}]
...
(defrecord ChWebSocket
  ...
  (-chsk-disconnect! [chsk reason]
    ...
    (when-let [cws @client-wsocket_]
      (interfaces/cws-close! cws)))
    ...
    (let [{?timeout-ms :timeout-ms ?cb :cb :keys [flush?]} opts
      ...
      (if-not (:open? @state_) ; Definitely closed
        ...
        (let [?cb-uuid (when ?cb-fn (enc/uuid-str 6))
          ...
          (try
            (interfaces/cws-send! @client-wsocket_ ppstr)
          ...
          (fn connect-fn []
            (when (have-handle?)
              (let [retry-fn
                      ...
                      (when (have-handle?)
                        (let [retry-count* (swap! retry-count_ inc)
                          ...
                          #?(:cljs (.setTimeout goog/global connect-fn backoff-ms)
                             :clj (future (Thread/sleep backoff-ms) (connect-fn))) ;; avoid thread?
                          (swap-chsk-state! chsk
                                            #(assoc % :udt-next-reconnect udt-next-reconnect)))))

                    ;; nha - difference with the original sente - this comes
                    ;; from the server-side PR
                    ;; url (WebSocket. (enc/...))
                    url (enc/merge-url-with-query-string
                          url
                          (merge params ; 1st (don't clobber impl.):
                                 {:client-id client-id
                                  :csrf-token (:csrf-token @state_)}))
                    ;; those are sente's callback functions
                    ;; that take care of reconnections and
                    ...
                    on-close (fn [ws-ev]
                               #?(:cljs (let [clean? (enc/oget ws-ev "wasClean")
                                          ...
                                          (if clean?
                                            ...
                                            (do
                                              ...
                                              (retry-fn))))
                                  :clj (let [last-ws-close {:udt    (enc/now-udt)
                                                            :ev     ws-ev}]
                                         (swap-chsk-state! chsk
                                                           #(assoc (chsk-state->closed % :unexpected)
                                                                   :last-ws-close last-ws-close)))))
                    on-msg (fn [ppstr]  ; Nb receives both push & cb evs!
                             (let [
                               ...
                               (or
                                 ...
                                 (if-let [cb-uuid ?cb-uuid]
                                   (if-let [cb-fn (pull-unused-cb-fn! cbs-waiting_
                                                                      cb-uuid)]
                                     (cb-fn clj)
                                     ;; TODO - I see this in the driver_test logs
                                     ;; could it be related to the ACK sente
                                     ;; callback not being in use at the moment?
                                     (warnf "Cb reply w/o local cb-fn: %s" clj))
                                   (let [buffered-evs clj]
                                     (receive-buffered-evs! chs buffered-evs))))))
                    ?socket (cws-creator url {:on-msg on-msg
                                              :on-error on-error
                                              :on-close on-close})]
                ...
                (if-not ?socket
                  ...
                  (do (assert (satisfies? interfaces/IClientWebSocket ?socket))
                      ;;#?(:cljs (js/console.log ?socket))
                      (reset! client-wsocket_ ?socket))))))]
...
#?(:clj (defn jvm-websocket-creator []
          (throw (Exception. "Missing websocket creator."))))
                       ...
                       (fn [] ; Backoff then recur
                         (when (have-handle?)
                           (let [retry-count* (inc retry-count)
                             ...
                             (swap-chsk-state! chsk
                                               #(assoc % :udt-next-reconnect udt-next-reconnect)))))]
                   ;; TODO try https://github.com/cljsjs/packages/blob/master/axios/resources/cljsjs/axios/common/axios.ext.js
                   ;; and reimplent it in CLJ too (aleph)?
                   (reset! curr-xhr_
                           (ajax-lite url
                                      (merge ajax-opts
                                      ...
                                      (fn ajax-cb [{:keys [?error ?content raw-resp xhr] :as reply}]
                                        ;; (timbre/error " AJAX XB " reply)
;;                                         (timbre/error " AJAX XB RESP TYPE " (.-responseType xhr))
;;                                         ;; https://developers.google.com/closure/library/docs/xhrio
;;                                         (timbre/error "XHR STATUS " (.getStatus xhr))
;;                                         (timbre/error "XHR IS ACTIVE " (.isActive xhr))
;;                                         (timbre/error "XHR isComplete " (.isComplete xhr))
;;                                         (timbre/error "XHR isSuccess" (.isSuccess xhr))
;;                                         (timbre/error "XHR getReadyState" (.getReadyState xhr))
;;                                         (timbre/error "XHR getLastUri" (.getLastUri xhr))
;;                                         (timbre/error "XHR getLastError" (.getLastError xhr))
;;                                         ;; XHR getLastError TypeError: bw.setTimeout is not a function
;;                                         (timbre/error "XHR HEADER "  (.getResponseHeader xhr "content-type"))
;;                                         (timbre/error "XHR RESP TEXT " (.getResponseText xhr))
;;                                         (timbre/error "STRING "  (js/String.fromCharCode.apply nil (js/Uint8Array. raw-resp)))
;;                                         (timbre/error " AJAX XB RESP " (type raw-resp) (type xhr))
;;                                         (timbre/error " AJAX XB RAW RESP  " ())
;;                                         (timbre/error " AJAX XB RESP " (.-response xhr))
                                        (if ?error
                                          (cond
...
(defn- new-ChAjaxSocket [opts csrf-token]
  #?(:clj
     ;; FIXME use aleph here too
     (throw (Exception. "No JVM ajax socket implemented."))
     :cljs (map->ChAjaxSocket
             (merge
...
(defn make-channel-socket-client!
  ...
  [path ?csrf-token {:keys [type protocol host port params packer ajax-opts ws-kalive-ms cws-creator
                            recv-buf-or-n client-id backoff-ms-fn]
                     :as   opts
                     :or   {type           :auto
                            ...
                            cws-creator #?(:cljs js-websocket-creator
                                           :clj jvm-websocket-creator)}}]
  (have? [:in #{:ajax :ws :auto}] type)
  (have? enc/nblank-str? client-id)

  ;; TODO check where this check is
  #_(when (str/blank? ?csrf-token)
    (warnf "WARNING: no CSRF token provided. Connections will FAIL if server-side CSRF check is enabled (as it is by default)."))

  (when-not packer (timbre/error "No packer provided"))

  (let [[ws-url ajax-url] (let [;; Not available with React Native, etc.:
                                win-loc  #?(:cljs (enc/get-win-loc) :clj nil)
                                path     (or path (:pathname win-loc))
                                protocol (or protocol (:protocol win-loc) :https)
                                host     (or host (:host     win-loc))
                                ;; TODO see if this is useful from original change
                                ;; https://github.com/ptaoussanis/sente/commit/9bb09acbf6f969e869b4736846107a54a32860ed
                                ;; (if port
        ...
        {:internal (chan (async/sliding-buffer 128))
         ...
         :<server (chan (async/sliding-buffer 512) (map (fn [ev] [:chsk/recv ev])))}
...
(defn start-server-chsk-router!
  ...
  [ch-recv event-msg-handler
   ;; NOTE(nha): this signature has been altered
   {:keys [trace-evs? error-handler simple-auto-threading?] :as opts}]
  ;; TODO return stop! fn in a map - not only the fn
